...
function deleteProvider(obj){
	...
	$.ajax({
		type:"POST",
		url:"/supermarket/provider/deleteprovider.action?proid="+obj.attr("proid"),
		dataType:"json",
...
$(function(){
	...
	*/
	...
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/provider/proview.action?proid="+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/provider/updateprovider.action?proid="+ obj.attr("proid");
	});
