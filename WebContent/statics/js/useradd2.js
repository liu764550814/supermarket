...
$(function(){
	...
	a_workPicPath=$("#a_idPicPath");
	errorinfo_wp = $("#a_workPicPath");
	addBtn = $("#add");
	backBtn = $("#back");
	...
	$.ajax({		
		type:"POST",//请求类型
		url:"/supermarket/role/rolelistNoCondition.action",//请求的url
	...
	userCode.bind("blur",function(){
		...
		$.ajax({
			type:"POST",//请求类型
			url:"/supermarket/user/ucexist.action",//请求的url
			data:{"userCode":userCode.val()},//请求参数
			dataType:"json",//ajax接口（请求url）返回的数据类型
			success:function(data){//data：返回数据（json对象）
				if(userCode.val() == null || userCode.val() == ""){
					validateTip(userCode.next(),{"color":"red"},imgNo+ " 该用户账号不可以为空",false);
				}
...
/*		if(referer != undefined 
			...
			var url = "/supermarket/user/userlist.action";
...
//});
	...
	a_workPicPath.blur(function(){
		if(a_workPicPath.val()==null||a_workPicPath.val()==""){
			a_workPicPath.next().html("* 上传大小不能超过500KB * 上传文件类型必须为：jpg、jpeg、png、pneg");
		}else{
			a_workPicPath.next().html(a_workPicPath.val());
		}
	
	});
	
	
	errorinfo_wp.blur(function(){
		if(errorinfo_wp.val()==null||errorinfo_wp.val()==""){
			errorinfo_wp.next().html("* 上传大小不能超过500KB * 上传文件类型必须为：jpg、jpeg、png、pneg");
		}else{
			errorinfo_wp.next().html(errorinfo_wp.val());
		}
