...
function deleteBill(obj){
	$.ajax({
		type:"POST",
		url:"/supermarket/bill/deletebillbyid.action?billid="+obj.attr("billid"),
		dataType:"json",
...
function changeDLGContent(contentStr){
 ...
 var ProviderId=$("#ProviderId");
			...
			success:function(data){//data：返回数据（json对象）
				if(data.providerlist != null){
					...
					for(var i = 0; i < data.providerlist.length; i++){
						if (data.providerlist[i].id == ProviderId.val()) {
							options += "<option selected value=\""+data.providerlist[i].id+"\">"+data.providerlist[i].proname+"</option>";
						} else {
	...
	$(".viewBill").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/bill/view.action?billid="+ obj.attr("billid");
	});
	
	$(".modifyBill").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/bill/updatebill.action?billid="+ obj.attr("billid");
	});
	$('#no').click(function () {
		cancleBtn();
	});
